...
import { normalizeToLF, resolveMutationTargetPath, stripBom } from "../fs-write";
	...
	return {
		...
		async execute(_toolCallId, params, signal?) {
			...
			const searchPath = params.path
				? resolveToCwd(params.path, env.cwd)
				: env.cwd;
			...
			for (const [filePath, matchLines] of matchesByFile) {
				...
				const displayPath = filePath.startsWith(env.cwd + "/")
					? filePath.slice(env.cwd.length + 1)
					: filePath;
