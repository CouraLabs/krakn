...
/**
 * Hash configuration is not configurable — the hashline engine always uses
 * the built-in defaults below. There is no config module; these constants are
 * the single source of truth.
 */
export const DEFAULT_HASH_LENGTH = 2;

/** Minimum supported hash length. */
export const HASH_LENGTH_MIN = 2;

/** Maximum supported hash length (length of {@link EXAMPLE_HASH}). */
export const HASH_LENGTH_MAX = 4;

/**
 * Example hash alphabet sequence used when constructing human-readable anchor
 * examples ("MQ" at the default length). Padded from the later characters when
 * rendering longer examples.
 */
export const EXAMPLE_HASH = "MQQV";

/**
 * Return the (fixed) hash size.
 * @returns HashLength: number
 */
...
/**
 * Example anchor hash shown in error messages and prompts, sized to the
 * configured hash length.
 */
export function exampleAnchor(): string {
	return EXAMPLE_HASH.slice(0, getHashLength());
}
